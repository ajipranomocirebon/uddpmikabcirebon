...
document.getElementById('btnTambahEpi').addEventListener('click', ()=>{
  epiRows.push({rid:epiSeq++, nomorKantong:'', parameters:[]});
  renderEpiRows();
});
...
formKegiatan.addEventListener('submit', async (e)=>{
  ...
  // Validasi data reaktif: tiap nomor kantong wajib diisi & minimal 1
  ...
  const payload = {
    ...
    epi: epiRows.map(r=>({nomorKantong:r.nomorKantong.trim(), parameters:[...r.parameters]}))
  };
...
function showEpiDetail(id){
  ...
  if(k.epi.length===0){
    tbody.innerHTML = `<tr class="empty-row"><td colspan="2">Tidak ada nomor kantong reaktif pada kegiatan ini.</td></tr>`;
  }else{
    tbody.innerHTML = k.epi.map(r=>{
      const {nomorKantong, parameters} = normalizeEpiRow(r);
      return `
      <tr>
...
function getEpiDetailExportData(id){
  ...
  const epiRows = k.epi.map(r=>{
    const {nomorKantong, parameters} = normalizeEpiRow(r);
    return { nomorKantong: nomorKantong || '—', parameter: parameters.join(', ') || '—' };
  });
  return {
...
function downloadEpiDetailExcel(){
  ...
  const aoa = [
    ...
    ['No. Kantong','Parameter Reaktif']
  ];
  if(d.epiRows.length===0){
    aoa.push(['Tidak ada nomor kantong reaktif pada kegiatan ini.','']);
  }else{
    d.epiRows.forEach(r=> aoa.push([r.nomorKantong, r.parameter]));
  }
...
function downloadEpiDetailPdf(){
  ...
  doc.autoTable({
    startY: y,
    head: [['No. Kantong','Parameter Reaktif']],
    body: d.epiRows.length ? d.epiRows.map(r=>[r.nomorKantong, r.parameter]) : [['Tidak ada nomor kantong reaktif pada kegiatan ini.','']],
    theme: 'grid', margin:{left:marginL,right:marginL}, styles:{fontSize:10},
